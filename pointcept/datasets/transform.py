...
class Collect(object):
    ...
    def __call__(self, data_dict):
        ...
        if data['offset'] < self.points:
            choice = np.random.choice(data['coord'].shape[0], self.points, replace=True)
        else:
            choice = np.random.choice(data['coord'].shape[0], self.points, replace=False)
              
        for key in data:
            if key == 'offset' or key == 'origin_offset':
                data[key] = torch.IntTensor([self.points])
                #print (key, data[key])
