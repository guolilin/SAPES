...
class SemSegTester(TesterBase):
    def test(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            dice_class = (2 * intersection) / (union + intersection + 1e-10)
            ...
            logger.info(
                ...
                "IoU {iou_V:.4f} {iou_A:.4f}"
                "Dice {dice_V:.4f} {dice_A:.4f}".format(
                    data_name,
                    idx + 1,
                    len(self.test_loader),
                    segment.size,
        ...
        if comm.is_main_process():
            ...
            logger.info(
                "Val result: mIoU/mdice {:.4f}/{:.4f}".format(
                    mIoU,mdice
                )
            )
            for i in range(self.cfg.data.num_classes):
                logger.info(
                    "Class_{idx} - {name} Result: iou/dice/ {iou:.4f}/{dice:.4f}".format(
                        idx=i,
...
class ClsVotingTester(TesterBase):
        ...
        for i in range(self.num_repeat):
            ...
            if comm.is_main_process():
                ...
                for m in self.best_record.keys():  
                    if isinstance(self.best_record[m], (float, int)): 
                        info += f"{m}: {self.best_record[m]:.4f} "  
                    elif m in ['class_accuracies', 'class_f1_scores']: 
                        info += f"{m}: [{', '.join(f'{x:.4f}' for x in self.best_record[m][:5]) if self.best_record[m] else 'N/A'}] "  
                    else:  
                        info += f"{m}: {self.best_record[m]} "
                logger.info(info)
    ...
    def test_once(self):
        ...
        for idx, data_dict in enumerate(self.test_loader):
            ...
            true_labels.extend(category.flatten())
        ...
        if comm.is_main_process():
            ...
            results = {  
                    'mAcc': mAcc,  
                    'allAcc': allAcc,  
                    'class_accuracies': accuracy_class.tolist(), 
                    'class_f1_scores': f1_per_class.tolist()   
                    } 
            logger.info("Val result:mAcc/allAcc/mF1_score {:.4f}/{:.4f}/{:.4f}".format(mAcc, allAcc, f1)) 
            for i in range(self.cfg.data.num_classes):
                logger.info(
                    "Class_{idx} - {name} Result: iou/accuracy/F1_score {accuracy:.4f}/{F1_score:.4f}".format(
                        idx=i,
                        name=self.cfg.data.names[i],
                        accuracy=accuracy_class[i],
